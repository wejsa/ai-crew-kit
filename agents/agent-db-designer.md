...
5. `_base/conventions/database.md` + `_base/checklists/architecture.md` 참조하여 설계 초안 작성

## 컨벤션 참조 (필수)

테이블/컬럼 네이밍, 필수 컬럼(`id`, `created_at`, `updated_at`, `deleted_at`), 제약조건 명명 패턴(`pk_`/`uk_`/`fk_`/`idx_`/`ck_`), 무중단 마이그레이션 원칙은 `_base/conventions/database.md`를 따릅니다. 본 에이전트는 **의사결정**과 **데이터 모델링 특수성**에 집중합니다.
