...
## v4.7.x → v4.8.0 마이그레이션 (에이전트 7종 재편 + 게이트 신호 A2 + 플러그인 모드 분기)

수동 작업 0건 — 업그레이드가 자동 처리합니다. (4.8.0의 나머지 변경 — 리뷰 인젝션 격리, 승인 영속화, 빌드 SSOT, M002 거짓 만료 해소, CC 기능 채택(maxTurns·disallowed-tools), 공개 대비 문서 — 은 프레임워크 내부 동작이라 별도 설정이 필요 없습니다.)
