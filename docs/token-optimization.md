...
절대 토큰 수치는 프로젝트 규모·diff 크기·대화 길이에 좌우되며, **공식 벤치마크는 아직 없습니다**. 어떤 절대 수치든 의심하고 보세요 — 아래는 "구조적으로 어디가 비싼가"에 대한 가이드입니다.

### 1 기능 루프의 비용 구조 (feature → plan → impl → review → merge)

| 단계 | 복잡도 클래스 | 비용 구조 |
|------|--------------|----------|
| feature | medium | 요구사항 문서 1회 생성 — 입력 서술 길이에 비례 |
| plan | heavy | 설계 분석 + 스텝 분해. `agents.enabled`에 db-designer 포함 시 서브에이전트 +1 |
| impl | medium × 스텝 수 | **가장 큰 변동 요인** — 스텝 수(보통 2~5)와 코드 규모에 비례. 기본 sonnet 라우팅 |
| review-pr | heavy (T2/T3) | **루프에서 가장 비싼 단계** — 서브에이전트 2~3개가 diff를 각자 읽음 + confidence 채점. 단, 작은 PR은 자동 Tier 분류가 T0(서브에이전트 0)·T1(1개)로 강등 |
| merge-pr | light | 상태 갱신 위주 |

### 비용을 줄이는 레버 (효과 순)

1. **PR을 작게** — 자동 Tier 분류의 직접 입력. 같은 변경량이면 작은 PR 여러 개(T0~T2)가 큰 PR 하나(T3 ×3 서브에이전트)보다 리뷰 비용이 낮은 경우가 많습니다.
2. **스킬 프로필** — `docs-only`/`developer` 프로필은 무거운 스킬 노출 자체를 줄입니다 (§6 참조).
3. **모델 라우팅** — impl=sonnet은 기본값. 리뷰 채점의 opus 핀은 게이트 신뢰성 장치이므로 유지 권장 — 내려도 절감은 작고(비싼 부분은 diff 읽기) 탐지 신뢰성만 깎입니다.
4. **200K 컨텍스트 운용** — 1M 세션은 같은 모델이라도 롱컨텍스트 프리미엄 + 맥락 누적으로 더 비쌉니다. 리뷰는 diff+체크리스트 수천~1만 토큰 규모라 `/model opus`(200K)로 충분합니다 (§7 Q&A 참조).
5. **측정이 우선** — Claude Code `/usage`로 스킬 실행 전후를 비교하고, Anthropic Console 사용량 대시보드로 월간 추세를 보세요. 팀 도입 판단은 추정치가 아니라 1~2주 실측으로.
