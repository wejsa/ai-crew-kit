...
> **채택안**: **옵션 B (메커니즘 + common + 도메인별 high confidence)** + **B-2 (D0 부채는 선분리 PR)**
...
2. 콘텐츠는 **high confidence 패턴만** 포함 (false positive 통제). medium/low는 v2.1+로 이관.
...
| **D4** | 옵션 B: high confidence만 포함, medium/low 보류 | Step 3 |
...
| `confidence` | ✅ | `high` / `medium` / `low` — 옵션 B는 high만 포함 |
...
- `.claude/domains/_base/health/secrets-patterns.json` (신규, ~150줄):
  - `common.hardcoded`: 5 high confidence 패턴
    ...
    - SEC-S03: AWS Access Key (`AKIA[0-9A-Z]{16}`)
    - SEC-S04: GitHub PAT (`ghp_[A-Za-z0-9]{36}`)
    - SEC-S05: Slack Bot Token (`xox[bp]-[\w-]+`)
  - `common.runtime`: SEC-01 12 패턴 외부화 (Step 2에서 SKILL.md가 이를 로드)
...
- [ ] **medium/low confidence 패턴 0개** (옵션 B)
