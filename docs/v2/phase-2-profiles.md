...
2. **SKILL.md 선택적 로드 실현 가능성 (H007 — 선행 확인 필수)**:
   - Claude Code가 `.claude/skills/*/SKILL.md`를 자동 로드하는 메커니즘 확인
   - **기본안 (CLAUDE.md 수준 필터링)**: SKILL.md는 전체 로드하되, CLAUDE.md.tmpl의 "주요 스킬" 섹션에서 프로파일에 따라 **노출/비노출을 제어** → Claude가 비노출 스킬을 자연어로 호출하지 않게 유도
   - **이상안 (SKILL.md 선택적 로드)**: Claude Code가 특정 SKILL.md를 무시하는 설정 지원 시 활용
   - TFT는 **기본안으로 설계를 진행**하되, 이상안이 가능할 경우 전환할 것
   - 기본안의 한계: 사용자가 직접 `/skill-xxx`를 호출하면 프로파일 무관하게 실행됨 → 이것은 의도된 동작 (프로파일은 가이드, 강제가 아님)
...
| Claude Code가 SKILL.md 선택 로드를 지원하지 않을 수 있음 | 중 | 높음 | **기본안(CLAUDE.md 필터링)으로 설계 진행** — 이상안 가능 시 전환 |
