...
| 1 — schema + validator + workflow job + plan + phase-7-context 보류 헤더 | ⏳ 진행 중 | feature/phase-7-step-1 |
| 2 — skill-retro §5.3 secrets 필터 통합 + impact 임계값 정량 출력 + tests/lessons/ pytest fixture | ⏳ | Step 1 머지 후 |
...
| **D2** | secrets 필터링은 Phase 5 `_base/health/secrets-patterns.json` `common.hardcoded` (SEC-S01~S05) 재사용. validator + skill-retro 양쪽 적용 | Step 1, 2 |
| **D3** | impact 임계값은 schema description으로 SSOT 명시 (`>=5 → high / >=3 → medium / <3 → low`). validator는 WARN only(사용자 override 허용) | Step 1, 2 |
| **D4** | cross-ref 검증은 completed.json + backlog.json(`tasks` + `archived`) 양쪽 ID 합집합. CRITICAL FAIL | Step 1 |
...
### Step 1: schema + validator + workflow + plan (PR 1)

**파일**:

- `.claude/schemas/lessons-learned.schema.json` (신규, ~80줄):
...
- `scripts/validate-lessons-learned.py` (신규, ~120줄):
  - `.claude/state/lessons-learned.json` 부재 시 graceful skip
  - jsonschema Draft 7 검증
  - taskId cross-ref (completed.json + backlog.json/tasks + archived 합집합)
  - description secrets 매칭 (SEC-S01~S05)
  - impact 임계값 sanity (WARN only)
  - 메타 레포(completed/backlog 부재) 시 cross-ref skip 안내

- `.github/workflows/secrets-tests.yml`:
  - `validate-lessons-learned` job 추가 (Python 3.11 + jsonschema 4.21.1)
...
**검증 (Step 1 로컬 실행)**:
- 부재 시 graceful skip (exit 0) ✅
- 정상 1건 → schema OK, 0 fail (cross-ref skip 안내 포함) ✅
- 잘못된 id `BAD-ID` → schema FAIL with `^L-\d{3,}$` mismatch ✅
- description에 `api_key='AbCd...'` + `AKIAI...` 박힘 → SEC-S01 + SEC-S03 matches → 2 CRITICAL fail ✅
- appliedCount=7 + impact=low → impact threshold WARN ✅

### Step 2: skill-retro 통합 + 임계값 정량 출력 + fixture (PR 2)

**파일**:
- `.claude/skills/skill-retro/SKILL.md`:
  - §5.3 저장 직전 `description` SEC-S01~S05 매칭 시 AskUserQuestion으로 사용자에게 알리고 거부 또는 마스킹 후 저장 옵션 제공
...
- `tests/lessons/test_secrets_filter.py` — SEC-S01~S05 각 1건 + clean 1건
...
**검증**:
- 12+ pytest 케이스 PASS
- 회귀 (schema 변경/임계값 변경) 시 FAIL 발생 확인
...
| **R2** | secrets 필터 false positive (코드 예시가 description에 정상 포함) | 중 | 낮 | excludeContexts 재사용 검토 + Step 2 AskUserQuestion으로 override 허용 | Step 2 |
| **R3** | cross-ref 검증이 archived Task 누락 시 false positive | 중 | 중 | completed.json + backlog.json `archived` 양쪽 검색. 양쪽 부재 시 SKIP | Step 1 |
