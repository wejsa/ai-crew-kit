"""Schema compliance regression for aick-init outputs.

Issue #62 — PR #61 (aick-init 재설계) 자체 정적 검증 13 케이스 영속화.
...
def test_task_template_omits_six_dynamic_fields(base_task: dict, task_schema: dict) -> None:
    """Step 9 task는 aick-impl이 동적 산정할 6 필드를 omit해야 함.

    init이 lockTTL=3600 박으면 aick-impl 동적 산정(≤3→3600, 4~8→7200, ≥9→10800)이
...
def test_workflowstate_gate_fields_pass_and_typed(backlog_schema: dict) -> None:
    ...
    # step.prNumber(SSOT — aick-impl Step 8)도 정수
...
WORKFLOWSTATE_TEMPLATE_FILES = [
    ...
    REPO_ROOT / ".claude/skills/aick-backlog/SKILL.md",
]
