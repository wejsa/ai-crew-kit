...
### Added
- **게시 라벨 self-check** (`aick-review-pr` Step 5): 인라인 코멘트 게시 직후 comment id로 재조회해 라벨 SSOT(첫 줄 `**CRITICAL/MAJOR/MINOR**` + 강등 마커 동일 줄) 준수를 검증 — 불일치 시 라벨 줄만 PATCH 교정 1회 → 실패 시 요약에 경고 게시(비차단·재조회 실패 시 스킵). `aick-fix` Step 2에 경고 인지 교차 참조. 게시-파싱 prose 정규식 페어의 드리프트를 게시 시점에 조기 발견.
- **상태 타임스탬프 형식 검증** (`aick-validate` 5.5 + `--fix`): backlog 시각 필드 8종 `fromdateiso8601` 파싱 시뮬레이션 — 파싱 불가 WARNING, `--fix`는 만료 관여 필드만 교정(`lockedAt`/`assignedAt`→현재 시각, `planApprovedAt`→null 재승인 강제, 이력 필드는 경고만). `diagnose.sh`에 파싱 불가 잠금 카운트 가시화(>0일 때만).
...
- **A2 운영 수칙 명문화**: "ad-hoc PR(핫픽스 포함)은 리뷰한 머신/세션에서 머지까지 완료 — 타 머신 머지는 신호 B에만 의존"을 merge-gate-explained §7(EN·KO 패리티) + `aick-hotfix` 주의사항 + `aick-review-pr` Step 6.5에 명시.
