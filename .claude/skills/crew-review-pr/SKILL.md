---
name: crew-review-pr
description: PR 리뷰 - GitHub PR에 대한 다관점 통합 리뷰 수행. 사용자가 "PR 리뷰해줘" 또는 /crew-review-pr을 요청할 때 사용합니다.
...
# crew-review-pr: PR 리뷰

## 실행 조건
- 사용자가 `/crew-review-pr {번호}` 또는 "PR {번호} 리뷰해줘" 요청 시
- `--auto-fix`: CRITICAL 이슈 자동 수정 후 재리뷰
- `--mode standard|full`: 이번 PR만 지정 모드로 리뷰 (일회성)
- `/crew-review-pr config`: 리뷰 모드 설정 관리

## 리뷰 모드 설정 (config 서브커맨드)

`/crew-review-pr config` 로 진입. 일반 리뷰 플로우와 분기된다.
...
## 워크플로우 상태 추적
CLAUDE.md 상태 추적 패턴. currentSkill="crew-review-pr"
...
> **디폴트 값 SSOT**: `project.schema.json`의 `properties.review.thresholds.{critical,major,minor}.default` 필드. 본 문서가 인용하는 80/60/50은 **schema 기본값의 사본**이며, schema 갱신 시 본 문서의 모든 80/60/50 표기를 함께 갱신해야 한다 (`grep -nE '\\b(80|60|50)\\b' .claude/skills/crew-review-pr/SKILL.md`로 위치 확인). 임계치 schema가 critical 최소 50을 강제 (project.schema.json).
...
#### 인라인 코멘트 라벨 형식 (SSOT — crew-fix 파싱 계약)

인라인 코멘트(`gh api .../pulls/{N}/comments`)의 본문 **첫 줄**은 다음 라벨로 시작한다. **본 섹션이 최종 게시 라벨 형식의 단일 진실 소스**이며, `crew-fix` Step 2가 이 형식을 정규식으로 파싱한다. sub-agent가 emit하는 markdown 표 셀 텍스트(`CRITICAL`/`MAJOR`/`MINOR`)는 중간 산출물일 뿐 — 인라인 코멘트로 게시될 때의 라벨은 본 SSOT가 결정한다.
...
- **강등 항목**은 MAJOR 라벨로 렌더하되 **반드시 `[원래 CRITICAL · 강등]` 마커를 라벨과 같은 줄에 병기**한다. 라벨 자체는 MAJOR이므로 crew-fix는 이 마커로만 강등을 식별한다(라벨만으로는 정상 MAJOR와 구분 불가).
...
- 이모지(`🔴`/`🟠`/`🟡`)는 시각 보조이며, crew-fix 파싱은 `**CRITICAL**` 볼드 토큰과 `[원래 CRITICAL · 강등]` 마커를 기준으로 한다(이모지 누락에도 견고하도록).
...
| 조건 | 결정 | chain |
|------|------|------|
| CRITICAL(필터 후) ≥1 | `gh pr review --request-changes` (자기 PR 여부 무관) | --auto-fix 모드면 crew-fix loop, 아니면 종료 |
...
- **workflowState.lastReviewDecision 갱신** (crew-fix 모드 판정 SSOT): 본 회차 결정값을 `APPROVED` / `COMMENT` / `REQUEST_CHANGES` 중 하나로 저장. crew-fix가 auto-fix vs 수동 호출 모드를 정확히 분기하기 위함(fixLoopCount 단독으로는 직전 루프 잔재가 잘못 분류).
...
#### 기본 모드 (--auto-fix 미사용)
- Step 6 결정 = APPROVE + chain 진행 → `Skill tool: skill="crew-merge-pr", args="{prNumber}"`
- Step 6 결정 = COMMENT + chain 진행 → `Skill tool: skill="crew-merge-pr", args="{prNumber}"` (자기 PR + 강등 0 케이스만)
...
- Step 6 결정 = REQUEST_CHANGES → `workflowState.fixLoopCount` 증가 후 `Skill tool: skill="crew-fix", args="{prNumber}"`
  - fixLoopCount 3회째 REQUEST_CHANGES → crew-fix 호출 금지, 즉시 중단 (루프 가드).
  - 직접 코드 수정 금지. crew-fix 없이 종료 금지.
