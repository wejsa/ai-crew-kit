---
name: crew-init
description: 프로젝트 초기화 - 요구사항 입력 → 스택 추천 → 백로그 자동 생성. /crew-init으로 호출합니다.
...
빈 디렉토리에서 새 프로젝트를 시작할 때 사용. **이미 코드가 있는 프로젝트는 `/crew-onboard`를 사용하세요** (자동 스캔 기반).

## 실행 조건
- 사용자가 `/crew-init` 또는 "프로젝트 시작해줘" 요청 시

## 옵션
```
/crew-init           # 새 프로젝트 초기화 (요구사항 우선 대화형)
/crew-init --quick   # 제로 결정 빠른 초기화 (자동 감지 + 기본값)
/crew-init --reset   # 기존 설정 초기화 (재설정)
/crew-init --quick --reset
```
...
| Step 11 완료 안내 | 그대로 + 백로그 시작 가이드 | + "/crew-init --reset" 안내 |
...
본격 진행 전 PostToolUse hook의 자동 비활성화(10초/3회 임계)를 일시 차단합니다. crew-init은 다수 Write를 짧은 시간에 발생시키므로 기본 임계값에서 false-positive 비활성화가 발생하기 쉽습니다.
...
| **기존 코드 파일** (자동 정리 SKIP된 경우만 평가) | `src/`, `app/`, `lib/`, 빌드 파일 등 감지 | `crew-onboard` 권장 안내 + 계속 진행 여부 확인 (Y=진행, N=중단) |
...
```
⚠ 기존 코드 파일이 감지되었습니다.
  crew-init은 신규 프로젝트(빈 디렉토리)용입니다.
  기존 코드베이스라면 /crew-onboard 가 더 적합합니다.

  계속 crew-init으로 진행하시겠습니까? [y/N]
```
...
ℹ 요구사항 정보가 부족하여 백로그 자동 분해를 건너뜁니다.
  /crew-feature 또는 /crew-backlog로 task를 직접 추가하세요.
```
...
> **init이 채우지 않는 필드** (crew-plan/crew-impl이 task 픽업/진행 시점에 동적 산정):
> - `assignee`, `assignedAt`: 픽업 시점에 crew-plan이 채움 (init 시점엔 미정)
> - `lockTTL`: crew-impl이 `lockedFiles` 수에 따라 동적 산정 (≤3→3600, 4~8→7200, ≥9→10800). init이 박으면 동적 산정 무력화 → 대형 task 동시성 사고 위험.
> - `lockedFiles`: crew-plan이 step별 파일 결정 시 채움
> - `steps`: crew-plan 영역 (Phase 4 비범위)
> - `workflowState`: crew-plan/impl이 진행 시 채움
> - `currentStep`: schema `minimum: 1`이라 0 불가. crew-plan이 픽업 시 1로 설정.
> - `specFile`: schema `type: "string"`이라 null 불가. init 시점엔 spec 없음. crew-feature/crew-plan이 필요 시 채움.
...
> **task.phase ↔ phases 키 매핑 규칙 (정합성 강제)**: `task.phase`는 정수 (1~4), `phases`의 키는 정수의 문자열 (`"1"`~`"4"`). crew-plan/crew-backlog가 phase grouping 시 다음 매핑을 따름:
> ```
...
1. **project.json** (경로: `.claude/state/project.json`):
   ...
   - **v2.0 GA 신규 필드**:
     ...
     - `metadata: { "version": 1, "createdAt": "<ISO8601>", "updatedAt": "<ISO8601>" }`
       ...
       - migrations.json에는 부재 — init이 schema 정합으로 생성 (`crew-upgrade` 결과물에는 없을 수 있음)
   - **`kitSource` 결정 규칙**:
...
- **백로그 시작 가이드** (Step 9 결과 task 수 > 0 인 경우만):
  ...
  ✓ 백로그에 {N}개 task가 준비되었습니다.
    /crew-plan 으로 첫 번째 task부터 시작하세요.
  ```
...
- **v1 데이터 복원 안내** (--reset 모드 + 백업 디렉토리에 v1 형식 backlog.json 감지 시: `kitVersion 부재` 또는 `< 2.0.0`):
  ...
  ⚠ v1 프로젝트 데이터가 백업되었습니다. 새 backlog.json은 v2 빈 백로그로 시작합니다.
    ...
    /crew-backlog로 수동 추가하세요. (v1→v2 자동 변환은 향후 지원 예정)
  ```
- 다음 단계 (`/crew-feature`, `/crew-backlog`, `/crew-docs`)
