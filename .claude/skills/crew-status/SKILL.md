...
## 실행 조건
- 사용자가 `/crew-status` 또는 "상태 확인해줘" 요청 시

## 명령어 옵션
```
/crew-status                  # 기본 상태 확인
/crew-status --locks          # 병렬 작업 잠금 현황 포함
/crew-status --health         # 시스템 건강 점검
/crew-status --health --fix   # 건강 점검 + Orphan Intent 자동 정리
```
...
Stale 감지 시 복구 안내: `/crew-impl` (재개) 또는 `/crew-backlog update {ID} --status=todo` (초기화)
...
문제 발견 시 원인+해결방법 출력 (예: `/crew-validate --fix`, `git pull origin develop`)

#### 에스컬레이션 안내
점검 결과에서 3개 이상 경고가 발견되면 출력 하단에 안내:
"💡 심층 검진이 필요할 수 있습니다: /crew-health-check"
...
- 7일 이상 경과: "📅 마지막 건강 검진이 {N}일 전입니다. /crew-health-check 권장"
- 14일 이상 경과: "⏰ 건강 검진이 {N}일간 미실행. /crew-health-check --fix 권장"
...
| 2 | PR APPROVED | `/crew-merge-pr {prNumber}` |
| 3 | PR CHANGES_REQUESTED | `/crew-fix {prNumber}` |
| 4 | PR 리뷰 미완료 | `/crew-review-pr {prNumber}` |
| 5 | in_progress + stale 30분+ | 복구 안내 |
| 6 | in_progress Task | `/crew-impl` |
| 7 | todo Task | `/crew-plan` |
| 8 | 모든 Task done | `/crew-feature "기능명"` |
...
| 스킬 | action | details |
|------|--------|---------|
| crew-impl | started, pr_created | stepNumber, prNumber |
| crew-review-pr | review_started, approved, request_changes, subagent_failed | prNumber, criticalCount, failedAgent 등 |
| crew-fix | fix_started, fix_completed | prNumber, issueCount |
| crew-merge-pr | merge_started, merged, task_completed | prNumber, stepNumber |
| crew-retro | retro_started, retro_completed, checklist_updated | reportFile, files |
| crew-hotfix | hotfix_started, hotfix_completed | description, hotfixId, prNumber, version |
| crew-rollback | rollback_started, rollback_completed | target, revertSha, prNumber, version |
| crew-status | orphan_intent_cleanup | method, pendingRecovered, intentFile |
