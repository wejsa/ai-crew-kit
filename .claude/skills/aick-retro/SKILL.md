---
name: aick-retro
description: 완료 Task 회고 - 분석 리포트 생성 + 체크리스트/컨벤션 학습 반영. 사용자가 "회고 해줘" 또는 /aick-retro를 요청할 때 사용합니다.
...
## 실행 조건
- `/aick-retro` — 최근 완료 1개 Task
- `/aick-retro {TASK-ID}` — 지정 Task
- `/aick-retro --summary` — completed.json 전체 통합 분석
- `/aick-retro --lessons [list|search {keyword}|top]` — lessons-learned.json 관리

## 사전 조건 (MUST-EXECUTE-FIRST — 하나라도 실패 시 STOP)
1. project.json 존재 → 없으면 "/aick-init 먼저 실행" 안내
2. completed.json 존재 + 유효 JSON → 파싱 실패 시 "/aick-validate --fix 실행" 안내
3. 완료된 Task 1건 이상 존재
...
- 매칭 시 AskUserQuestion (default = 거부):
  ...
  - **강행 저장**: 안티패턴 *예시* 등 의도된 경우 그대로 저장 (사용자 책임). 이후 CI `validate-lessons-learned` 통과 — Step 1 D2 개정으로 validator는 secrets 검사를 aick-retro 레이어로 위임
...
파일 미존재 시: "학습 항목이 없습니다. `/aick-retro` 먼저 실행하세요."
