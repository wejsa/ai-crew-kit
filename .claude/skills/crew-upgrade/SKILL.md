---
name: crew-upgrade
description: 프레임워크 업그레이드 - ai-crew-kit 최신 버전으로 프레임워크 파일 업데이트. /crew-upgrade로 호출합니다.
...
## 실행 조건
- 사용자가 `/crew-upgrade` 또는 "프레임워크 업그레이드해줘" 요청 시

## 옵션
```
/crew-upgrade                              # 최신 버전으로 업그레이드
/crew-upgrade --dry-run                    # 변경 사항 미리보기 (실제 변경 없음)
/crew-upgrade --source <git-url|local-path> # 소스 지정
/crew-upgrade --version <tag>               # 특정 버전으로 업그레이드
/crew-upgrade --rollback                    # 가장 최근 백업에서 롤백
/crew-upgrade --rollback <backup-path>      # 지정 백업에서 롤백
```
...
| 항목 | 조건 | 처리 |
|------|------|------|
| project.json | 없음 | "/crew-init 먼저 실행" 안내 후 종료 |
...
- ⚠️ **`hooks/` 스크립트는 clone/세션 시 자동 실행되는 보안 민감 파일**이다. 해시 불일치 hook은 미리보기에서 반드시 가시화하고, 교체 후 `crew-health-check`의 `hook-safety` 카테고리가 위험 패턴을 재검한다. 사용자가 직접 하드닝한 hook을 보존하려면 "현재 유지"를 선택한다(Step 11이 해당 파일 교체에서 제외).
...
`Skill tool: skill="crew-validate"` 자동 호출.
