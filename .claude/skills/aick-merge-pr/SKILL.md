---
name: aick-merge-pr
description: PR 머지 - 승인된 PR을 Squash 머지하고 상태 업데이트. 사용자가 "PR 머지해줘" 또는 /aick-merge-pr을 요청할 때 사용합니다.
...
## 실행 조건
- 사용자가 `/aick-merge-pr {번호}` 또는 "PR {번호} 머지해줘" 요청 시
...
## 워크플로우 상태 추적
CLAUDE.md 상태 추적 패턴. currentSkill="aick-merge-pr". Task 완료 시 workflowState=null.
...
### 4. 상태 업데이트
`aick-backlog` 쓰기 프로토콜 준수 (metadata.version +1, JSON 검증).
...
### 6. 다음 스텝 자동 진행
- 남은 스텝 있음: `Skill tool: skill="aick-impl", args="--next"`
