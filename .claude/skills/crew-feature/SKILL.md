...
## 실행 조건
- 사용자가 `/crew-feature {기능명}` 또는 "새 기능 기획해줘" 요청 시
- 기능명 없이 호출 시 기능 추천
...
### 7. crew-plan 자동 호출
"Y" 승인 시 반드시 `Skill tool: skill="crew-plan", args="{taskId}"` 호출.
직접 설계 진행 금지.
...
## 출력
필수 포함: Task ID/기능명/Phase/우선순위/예상 스텝, 요구사항 문서 경로, 승인 선택지(Y→백로그 등록+crew-plan / N→수정)
