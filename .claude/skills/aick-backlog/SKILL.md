---
name: aick-backlog
description: 백로그 관리 - Task 목록 조회, 추가, 수정, 우선순위 변경. /aick-backlog로 호출합니다.
...
## 실행 조건
- 사용자가 `/aick-backlog` 또는 "백로그 보여줘" 요청 시

## 명령어 옵션

| 명령어 | 예시 |
|--------|------|
| 조회 | `/aick-backlog`, `list`, `list --status=todo`, `list --phase=2` |
| 조회 확장 | `list --type=bug`, `list --assignee=me`, `list --stale=3d` |
| 추가 | `/aick-backlog add "제목" --phase=2 --priority=high` |
| 추가 확장 | `add "상품 검색 500 에러" --type=bug` |
| 수정 | `/aick-backlog update {taskId} --status=in_progress` |
| 수정 확장 | `update {taskId} --title="새 제목" --description="설명" --phase=2 --type=bug --reason="사유"` |
| 우선순위 | `/aick-backlog priority {taskId} high` |
| 잠금 해제 | `/aick-backlog unlock {taskId} --force` |
| 대시보드 | `/aick-backlog dashboard` |
| 보관 | `/aick-backlog archive {taskId}` |
| 일괄 변경 | `/aick-backlog batch --phase=1 --set-phase=2` |
| 의존성 | `/aick-backlog deps [taskId] [--reverse]` |
...
aick-status와의 차이: status는 Git/설정/워크플로우 기술 진단, dashboard는 Task 중심 진행률.
...
다중 Task 일괄 변경.
```
/aick-backlog batch --phase=1 --priority=medium --set-phase=2
/aick-backlog batch --status=blocked --set-priority=critical
/aick-backlog batch TASK-001,TASK-003 --set-status=todo
```
...
의존성 텍스트 트리.
```
/aick-backlog deps                    # 전체 의존성 트리
/aick-backlog deps TASK-003           # 특정 Task 의존 체인
/aick-backlog deps TASK-001 --reverse # 영향도 (이 Task에 의존하는 Task)
```
...
- `/aick-status`에서 경고 표시, 다른 세션에서 인계 가능
