---
name: crew-validate
description: 프레임워크 검증 - 업그레이드 후 구조 무결성 자체 검증. crew-upgrade에서 자동 호출되거나 /crew-validate로 호출합니다.
...
# crew-validate: 프레임워크 검증

## 실행 조건
- `crew-upgrade` 완료 후 자동 호출
- 또는 사용자가 `/crew-validate` 직접 호출

## 옵션
```
/crew-validate          # 검증만 수행 (읽기 전용)
/crew-validate --fix    # 자동 수정 가능한 항목 수정
```
