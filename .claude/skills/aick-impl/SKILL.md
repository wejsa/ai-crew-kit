---
name: aick-impl
description: 구현 - 스텝별 개발 + PR 생성. 사용자가 "개발 진행해줘", "구현해줘" 또는 /aick-impl을 요청할 때 사용합니다.
...
## 실행 조건
- 사용자가 `/aick-impl` 또는 "개발 진행해줘" 요청 시
...
## 워크플로우 상태 추적
CLAUDE.md 상태 추적 패턴. currentSkill="aick-impl"
...
### 8. 상태 업데이트
`aick-backlog` 쓰기 프로토콜 준수 (metadata.version +1, JSON 검증 필수).
...
### 9. 다음 스킬 호출 (프로필별)
- **standard**: `Skill tool: skill="aick-review-pr", args="{prNumber} --auto-fix"`
- **fast**: `Skill tool: skill="aick-merge-pr", args="{prNumber}"` (review 생략)
- 직접 리뷰/머지 금지. 반드시 Skill tool 사용.

> **플러그인 설치 시**: 스킬 이름은 `aick-*` bare 이름으로 표기하나, 플러그인으로 설치된 환경에서는 `ai-crew-kit:aick-review-pr`처럼 네임스페이스가 붙어 노출됩니다. 사용 가능한 스킬 목록의 네임스페이스된 이름으로 호출하세요. (clone/seed 설치는 bare 이름 그대로 동작 — 두 방식 모두 동일한 체이닝 의도.)
...
4. **라인 수 검증 (Micro 전용)**:
   ...
   - > 300줄: 차단 "Standard 경로 필요. /aick-plan 실행"
5. **빌드 & 테스트**: 일반과 동일
...
- 장시간 작업: **하트비트는 `lockedAt`** — 훅이 편집마다 자동 갱신하므로 `assignedAt`는 **불변**(할당 기록)으로 둔다(v4.5.0, schema 정합). 동적 TTL은 aick-backlog 참조.
