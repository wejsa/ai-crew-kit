---
name: crew-docs
description: 프로젝트 참고자료 - 개발 시 자동 참조되는 공통 컨벤션·문서. 사용자가 "문서 찾아줘", "참고자료" 또는 /crew-docs를 요청할 때 사용합니다.
...
## 개요
프로젝트 공통 컨벤션·참고자료 제공. 개발 중 관련 키워드 감지 시 자동 참조.

## 사용 방법

```
/crew-docs                    # 전체 문서 목록
/crew-docs payment            # 결제 관련 문서
/crew-docs api                # API 설계 문서
/crew-docs template api-spec  # 템플릿 출력
```
...
│   └── templates/      # 공통 템플릿
└── general/
    └── docs/           # 범용 참고자료 (getting-started·common-patterns 등)
```

**로딩 우선순위**: general/docs/ → _base/conventions/ → _base/templates/
...
### 문서 목록 조회
필수 포함: 범용 참고자료 테이블(문서/설명/키워드), 공통 컨벤션 테이블

### 특정 문서 조회
문서 내용 출력 + 관련 문서 목록

## 자동 참조 동작
1. 키워드 감지 → 2. 공통 컨벤션 키워드 매핑 탐색 → 3. 문서 읽기 → 4. 코드 반영 → 5. 출처 표기

## 커스터마이징
새 문서 추가: `.claude/domains/general/docs/` 디렉토리에 .md 추가 + `docs/README.md`에 키워드 매핑 추가
