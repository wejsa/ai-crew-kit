---
name: crew-release
description: 릴리스 - 빌드 검증 + API spec 스냅샷 + 버전 범프 + CHANGELOG + main 머지 + 태그 생성. /crew-release로 호출합니다.
...
# crew-release: 릴리스 자동화

## 실행 조건
- 사용자가 `/crew-release {버전타입}` 요청 시 (develop 브랜치에서만)
...
7. Health Gate (선택): project.json에 healthCheck 설정이 있으면
   /crew-health-check --quick 실행. CRITICAL 0건이어야 진행.
