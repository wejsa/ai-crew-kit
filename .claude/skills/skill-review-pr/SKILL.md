...
**Task 호출 옵션**:
- `isolation: "worktree"` — 각 리뷰어를 격리된 worktree에서 실행 (Claude Code v2.1.49+, 리뷰어 frontmatter에도 동일 설정 — 이중 안전)
- 리뷰어는 읽기 전용이므로 worktree가 자동 정리된다
- `/tmp/pr-{N}-diff.txt`는 worktree 외부 경로이므로 격리 환경에서도 동일하게 접근 가능
...
## 출력
필수 포함: PR 번호/제목/작성자/브랜치, **리뷰 모드 + 실행 에이전트 목록**, 체크리스트 결과, 관점별 리뷰 테이블(CRITICAL/MAJOR/MINOR 수), 주요 피드백 목록, 결정(APPROVED/REQUEST_CHANGES), 다음 자동 스킬
