---
name: crew-create
description: 커스텀 스킬 생성 - SKILL.md 스캐폴딩 + CLAUDE.md 자동 등록. 사용자가 "스킬 만들어줘" 또는 /crew-create를 요청할 때 사용합니다.
...
## 실행 조건
- 사용자가 `/crew-create {name} "{description}"` 또는 "커스텀 스킬 만들어줘" 요청 시
...
## 인수 파싱
`/crew-create <name> [description]`
- name: 영문 소문자+하이픈 `[a-z0-9-]`, `crew-` 접두사 자동 추가 (이미 있으면 유지)
- description: 생략 시 AskUserQuestion으로 수집

## 실행 플로우

### Step 1: 중복 검사
- `.claude/skills/custom/crew-{name}` 존재 → 덮어쓰기 여부 확인
- `.claude/skills/crew-{name}` (빌트인) 존재 → "이름 충돌" 안내 후 종료
...
### Step 3: SKILL.md 생성
`.claude/skills/custom/crew-{name}/SKILL.md` — 프론트매터(name, description, allowed-tools) + 기본 구조(실행 조건, 플로우, 출력, 주의사항)
