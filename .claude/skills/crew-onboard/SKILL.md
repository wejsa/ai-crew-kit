---
name: crew-onboard
description: 기존 프로젝트 온보딩 - 코드베이스 스캔 + 자동 설정 생성. 사용자가 "프로젝트 온보딩" 또는 /crew-onboard를 요청할 때 사용합니다.
...
# crew-onboard: 기존 프로젝트 온보딩

## 실행 조건
- 사용자가 `/crew-onboard` 또는 "이 프로젝트에 적용해줘" 요청 시
- **기존 코드베이스** 대상 (새 프로젝트는 `/crew-init`)

## 옵션
```
/crew-onboard              # 전체 온보딩 (스캔 + 설정 생성)
/crew-onboard --scan-only  # 스캔만 (분석 결과 출력 후 종료)
```
...
> **이 디렉토리에 코드 파일이 없다면 `/crew-init`을 사용하세요.** crew-onboard는 기존 코드베이스 자동 스캔이 핵심이라 빈 디렉토리에서 의미 있는 결과를 만들 수 없습니다.
...
PostToolUse hook의 자동 비활성화(10초/3회 임계)를 일시 차단합니다. crew-onboard는 다수 Write를 짧은 시간에 발생시키므로 기본 임계값에서 false-positive 비활성화가 발생하기 쉽습니다.
...
```
⚠ 코드 파일이 감지되지 않았습니다.
  crew-onboard는 기존 코드베이스 자동 스캔이 목적입니다.
  빈 디렉토리에서 신규 프로젝트를 시작하려면 /crew-init 을 사용하세요.

  계속 crew-onboard로 진행하시겠습니까? [y/N]
```
...
### Step 3: 추가 정보 수집
AskUserQuestion: 프로젝트 이름 (디렉토리명 기본값), 설명, 에이전트 구성 (crew-init Step 5 동일 — 스택 기반 필수 자동 결정 + 선택 추가), Task 접두사
...
### Step 5: 설정 파일 생성
crew-init Step 10 동일: project.json (buildCommands 포함), backlog.json (빈 값으로 초기화 — 자동 분해는 init 전용), CLAUDE.md, README.md, VERSION (기존 있으면 유지)
...
### Step 7: 완료 리포트
필수 포함: 프로젝트 정보, 생성된 파일, 백업된 파일, 다음 단계 (/crew-feature, /crew-backlog, /crew-plan)
