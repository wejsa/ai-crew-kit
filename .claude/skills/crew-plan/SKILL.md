---
name: crew-plan
description: 계획 수립 - Task 선택 + 설계 분석 + 스텝 분리 계획. 사용자가 "다음 작업 가져와", "계획 세워줘" 또는 /crew-plan을 요청할 때 사용합니다.
...
## 실행 조건
- 사용자가 `/crew-plan` 또는 "다음 작업 가져와" 요청 시
- 특정 Task 지정: `/crew-plan {taskId}`
...
## 워크플로우 상태 추적
CLAUDE.md 상태 추적 패턴. currentSkill="crew-plan"
...
### 8. crew-impl 자동 호출
승인 시 반드시 `Skill tool: skill="crew-impl"` 호출. 직접 개발 금지.
