---
name: crew-estimate
description: 작업 복잡도 추정 - 5팩터 분석 + 과거 데이터 보정. 사용자가 "작업량 추정해줘" 또는 /crew-estimate를 요청할 때 사용합니다.
...
## 실행 조건
- 사용자가 `/crew-estimate {TASK-ID}` 또는 "이 작업 얼마나 걸려?" 요청 시
- 사용자가 `/crew-estimate --phase {N}` 또는 "페이즈 N 분석해줘" 요청 시
- 사용자가 `/crew-estimate --sprint` 또는 "스프린트 플래닝 해줘" 요청 시

## 실행 모드

| 모드 | 트리거 | 대상 |
|------|--------|------|
| 단일 Task | `/crew-estimate TASK-001` | 지정 Task 복잡도 추정 |
| 페이즈 | `/crew-estimate --phase 1` | 해당 Phase의 모든 Task 요약 |
| 스프린트 | `/crew-estimate --sprint` | TODO 상태 Task 우선순위 + 추정 |
