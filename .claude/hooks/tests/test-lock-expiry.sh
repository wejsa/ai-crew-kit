...
# T1 만료(lockedAt) / T2 미만료(lockedAt) / T3 비-ISO8601(lockedAt)
# T4 lockedAt=null·assignedAt 만료 → 폴백 만료 / T5 lockedAt=null·assignedAt 미만료 → 폴백 유지
# T6 lockedAt=null·assignedAt 비-ISO8601(date-only) → 폴백 경로의 파싱 불가 — 만료 아님
cat > "$BACKLOG" <<EOF
{
  ...
  "tasks": {
    ...
    "T6": {"id": "T6", "status": "in_progress", "lockedAt": null, "assignedAt": "2026-06-12", "lockedBy": "frank@host", "lockTTL": 3600}
  }
}
EOF
...
t6b="$(jq -r '.tasks["T6"].lockedBy' "$BACKLOG")"
...
# T3 (비-ISO8601): 파싱 불가 → 만료로 취급하지 않음 (v4.8.0, M002 해소 — 구 동작은 `// 0` 폴백 즉시 거짓 만료)
assert_eq "garbage-not-iso" "$t3" "T3 (invalid ISO8601) — 만료 아님, 잠금 유지 (M002 해소)" || fail=$((fail + 1))
...
assert_eq "frank@host" "$t6b" "T6 (assignedAt date-only invalid → 폴백 경로 파싱 불가) lockedBy 유지됨" || fail=$((fail + 1))
